...
from pytorch_pretrained_bert.modeling import BertForDCMN
...
class RaceExample(object):
    ...
    def __init__(self,
        ...
        self.options = [option_0, option_1, option_2, option_3]
        self.label = label
...
class InputFeatures(object):
    def __init__(self, example_id, choices_features, label):
        ...
        self.choices_features = [
            ...
            for _, input_ids, input_mask, segment_ids, p_mask, q_mask, a_mask in choices_features
        ]
        self.label = label
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    # Format the input for each option to be:
    ...
    for example_index, example in enumerate(examples):
        ...
        for option_index, option in enumerate(example.options):
            ...
            a_len = len(option_tokens)
            truncated_article_length = min(len(article_tokens), max_seq_length - 4 - q_len - a_len)
            tokens = ["[CLS]"] + article_tokens[:truncated_article_length] + \
                    ["[SEP]"] + question_tokens + ["[SEP]"] + option_tokens + ["[SEP]"]

            input_ids = tokenizer.convert_tokens_to_ids(tokens)
            input_mask = [1] * len(input_ids)
            segment_ids = [0] * (truncated_article_length + 2) + [1] * (q_len + a_len + 2)
            p_mask = [0] + [1] * truncated_article_length + [0] * (q_len + a_len + 3)
            q_mask = [0] * (truncated_article_length + 2) + [1] * q_len + [0] * (a_len + 2)
            a_mask = [0] * (truncated_article_length + q_len + 3) + [1] * a_len + [0]
...
def main():
    ...
    if args.load_from_epoch == None:
        model = BertForDCMN.from_pretrained(args.bert_model,
    ...
    if args.do_train:
        ...
        if os.path.exists("./train_features.pkl"):
            with open("train_features.pkl", "rb") as f:
                train_examples, train_features, eval_examples, eval_features = pickle.load(f)
            ...
            with open('train_features.pkl', 'wb') as f:
                pickle.dump([train_examples, train_features, eval_examples, eval_features], f)
                        
        all_input_ids = torch.tensor(select_field(train_features, 'input_ids'), dtype=torch.long)
        all_input_mask = torch.tensor(select_field(train_features, 'input_mask'), dtype=torch.long)
        all_segment_ids = torch.tensor(select_field(train_features, 'segment_ids'), dtype=torch.long)
        all_p_mask = torch.tensor(select_field(train_features, 'p_mask'), dtype=torch.long)
        all_q_mask = torch.tensor(select_field(train_features, 'q_mask'), dtype=torch.long)
        all_a_mask = torch.tensor(select_field(train_features, 'a_mask'), dtype=torch.long)
        all_label = torch.tensor([f.label for f in train_features], dtype=torch.long)
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label,
                                   all_p_mask, all_q_mask, all_a_mask)
        if args.local_rank == -1:
        ...
        all_p_mask = torch.tensor(select_field(eval_features, 'p_mask'), dtype=torch.long)
        all_q_mask = torch.tensor(select_field(eval_features, 'q_mask'), dtype=torch.long)
        all_a_mask = torch.tensor(select_field(eval_features, 'a_mask'), dtype=torch.long)
        ...
        for ep in range(int(args.start_train_epoch)-1, int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                input_ids, input_mask, segment_ids, label_ids, p_mask, q_mask, a_mask = batch
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        ## test high 
        if os.path.exists("./test_high_features.pkl"):
            with open("test_high_features.pkl", "rb") as f:
                eval_examples, eval_features = pickle.load(f)
        else:
            eval_examples = read_race_examples(test_high)
            eval_features = convert_examples_to_features(
                eval_examples, tokenizer, args.max_seq_length, True)
            with open('test_high_features.pkl', 'wb') as f:
                pickle.dump([eval_examples, eval_features], f)                
        ...
        for step, batch in enumerate(eval_dataloader):
            batch = tuple(t.to(device) for t in batch)
            input_ids, input_mask, segment_ids, label_ids, ids, p_mask, q_mask, a_mask = batch
        ...
        ## test middle
        if os.path.exists("./test_middle_features.pkl"):
            with open("test_middle_features.pkl", "rb") as f:
                eval_examples, eval_features = pickle.load(f)
        else:
            eval_examples = read_race_examples(test_middle)
            eval_features = convert_examples_to_features(
                eval_examples, tokenizer, args.max_seq_length, True)
            with open('test_middle_features.pkl', 'wb') as f:
                pickle.dump([eval_examples, eval_features], f)
